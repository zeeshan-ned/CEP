...
class kbc():
    ...
    def start(self):
        ...
        correct_answers = ["Russia", "366", "Heron", "Dollar", "Python", "36",
                           ...
                           "Bill Gates"]
        self.S = th.Timer(6, self.thread)

        self.S.start()

        def playagain_win(self):
            self.label = False
        ...
        def on_closing_loose(self):
            self.root1.destroy()
            self.root.destroy()

        def tryagain_loose(self):
            # mixer.music.load('suspense.mpeg')
            # mixer.music.play(-1)
            # phoneLifelineButton.config(
            #     state=NORMAL, image=phoneImage)
            ...
            for i in range(15):
                if value == correct_answers[i]:
                    if value == third_option[14]:
                        ...
                        self.root2.geometry('500x400+140+30')
                        self.root2.title('You won 1 million Pounds')
                        centerimg = PhotoImage(file='center.png')
                        imgLabel = Label(self.root2, image=centerimg, bd=0, )
                        imgLabel.pack(pady=30)

                        winlabel = Label(self.root2, text='You Won', font=(
                            'arial', 40, 'bold'), bg='black', fg='white')
                        winlabel.pack()

                        happyimage = PhotoImage(file='happy.png')
                        happYLabel = Label(
                            self.root2, image=happyimage, bg='black')
                        ...
                        playagainButton = Button(self.root2, text='Play Again', font=('arial', 20, 'bold'), bg='black', fg='white',
                                                 bd=0, activebackground='black', cursor='hand2', activeforeground='white',
                                                 command=self.playagain_win)
                        playagainButton.pack()

                        closeButton = Button(self.root2, text='Close', font=('arial', 20, 'bold'), bg='black', fg='white', bd=0, activebackground='black', cursor='hand2', activeforeground='white',
                                             command=self.on_closing_win)
                        closeButton.pack()

                        self.root2.protocol(
                            'WM_DELETE_WINDOW', self.on_closing_win)
                        self.root2.mainloop()
                        break
                ...
                if value not in correct_answers or self.label == True:
                    ...
                    self.root1.geometry('500x400+140+30')
                    self.root1.title('You won 0 Pound')
                    img = PhotoImage(file='center.png')
                    imgLabel = Label(self.root1, image=img, bd=0)
                    imgLabel.pack(pady=30)
                    loselabel = Label(self.root1, text='You Lose', font=(
                        'arial', 40, 'bold'), bg='black', fg='white')
                    loselabel.pack()
                    sadimage = PhotoImage(file='sad.png')
                    sadlabel = Label(self.root1, image=sadimage, bg='black')
                    sadlabel.place(x=400, y=280)
                    sadlabel1 = Label(self.root1, image=sadimage, bg='black')
                    sadlabel1.place(x=30, y=280)

                    tryagainButton = Button(self.root1, text='Try Again', font=('arial', 20, 'bold'), bg='black', fg='white', bd=0, activebackground='black', cursor='hand2', activeforeground='white',
                                            command=self.tryagain_loose)
                    tryagainButton.pack()

                    closeButton = Button(self.root1, text='Close', font=('arial', 20, 'bold'), bg='black', fg='white', bd=0, activebackground='black', cursor='hand2', activeforeground='white',
                                         command=self.on_closing_loose)
                    closeButton.pack()
        ...
        self.root = Tk()
        self.root.geometry('1270x652+0+0')
        self.root.resizable(0, 0)
        self.root.title('Quiz Game')
        self.root.config(bg='black')

        leftFrame = Frame(self.root, bg='black', padx=90)
        leftFrame.grid(row=0, column=0)

        rightFrame = Frame(self.root, bg='black', padx=50, pady=25)
        ...
        callButton = Button(
            self.root, bg='black', bd=0, activebackground='black', cursor='hand2', command=phoneclick)
        ...
        progressbarA = Progressbar(
            self.root, orient=VERTICAL, mode='determinate', length=120)

        progressbarLabelA = Label(self.root, text='A', font=(
            'arial', 20, 'bold'), bg='black', fg='white')

        progressbarB = Progressbar(
            self.root, orient=VERTICAL, mode='determinate', length=120)

        progressbarLabelB = Label(self.root, text='B', font=(
            'arial', 20, 'bold'), bg='black', fg='white')

        progressbarC = Progressbar(
            self.root, orient=VERTICAL, mode='determinate', length=120)

        progressbarLabelC = Label(self.root, text='C', font=(
            'arial', 20, 'bold'), bg='black', fg='white')

        progressbarD = Progressbar(
            self.root, orient=VERTICAL, mode='determinate', length=120)

        progressbarLabelD = Label(self.root, text='D', font=(
            'arial', 20, 'bold'), bg='black', fg='white')
